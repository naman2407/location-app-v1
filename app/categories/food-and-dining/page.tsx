...
function FoodAndDiningContent() {
  ...
  // Convert brand names to card data and sort alphabetically
  const brandCards = useMemo(() => {
    const cardData = brandNamesToCardData(foodAndDiningBrands, isClaimedFoodBrand)
  ...
  const currentPage = Number.isFinite(pageParam) && pageParam > 0 ? Math.min(Math.max(pageParam, 1), totalPages) : 1
  ...
  const handlePageSizeChange = (newSize: number) => {
    ...
    router.push(`/categories/food-and-dining?${params.toString()}`)
  }
