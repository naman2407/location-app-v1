...
import { getBrandBySlug, getStateBySlug, countBrandLocations } from '../../../../constants/brandData'
...
export default function BrandStatePage({ params }: PageProps) {
  ...
  const description = brand.claimed
    ? `Quickly find the nearest ${brand.name} locations in ${state.name}, check whether it's open, and get accurate information like hours, reviews, and FAQs. This verified, up-to-date information about ${brand.name} locations comes straight from the Yext Knowledge Graph.`
    : `Quickly find the nearest ${brand.name} locations in ${state.name}, check whether it's open, and get accurate information like hours, reviews, and FAQs.`
              ...
              return (
                <li key={city.slug} className="mb-6">
                  {isClickable ? (
                    ...
                    <span className="text-[#5A58F2] font-medium cursor-default pointer-events-none">{city.name}</span>
                  )}
                </li>
              )
            })}
